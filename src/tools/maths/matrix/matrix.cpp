        ...
        for (uint32_t l_line = 0; l_line < m_size; l_line++)
        {
            if (l_line != l_column)
            {
                ...
                if (l_elimConst != 0)
                {
                    ...
                    for (uint32_t l_column2 = 0; l_column2 < 2 * m_size;
                         l_column2++)
                    {
                        l_juxMatrix[l_line][l_column2]
                            -= l_juxMatrix[l_elimLine][l_column2] * l_elimConst;
                    }
                    ...
                    {
                        l_juxMatrix[l_line][l_column2]
                            = l_juxMatrix[l_line][l_column2] / l_diagConst;
                    }
                }
            }
        }

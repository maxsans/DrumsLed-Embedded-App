...
static void udp_listen_thread()
{
    ...
    while (udp_running)
    {
        ...
        if (bytes_received > 0 && udp_recv_callback)
        {
            ...
            udp_recv_callback(buffer, bytes_received, client_ip, ntohs(client_addr.sin_port), client_mac);
        }
...
void udp_send_broadcast(const char *data, int16_t len, int16_t port)
{
    ...
    sendto(udp_socket, data, len, 0, (sockaddr*)&broadcast_addr, sizeof(broadcast_addr));
}

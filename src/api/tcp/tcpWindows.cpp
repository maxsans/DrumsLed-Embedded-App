...
{
    if (!winsock_initialized) tcp_init();
...
uint32_t tcp_recv(char *data, int16_t len, char *ip, int16_t *port, char *mac)
{
    ...
    if (!winsock_initialized || listenSock == INVALID_SOCKET) tcp_init();
    ...
    if (received > 0) {
        if (ip) inet_ntop(AF_INET, &clientAddr.sin_addr, ip, 16);
        if (port) *port = ntohs(clientAddr.sin_port);
        if (mac) memset(mac, 0, 6);
        return received;
    } else if (received == 0 || (received == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK)) {
